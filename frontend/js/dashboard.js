...
function renderExpansionResults(result, candidates, weightMap) {
  ...
  const rankColors  = ['#f1c40f', '#95a5a6', '#cd7f32', '#4a90e2', '#7b68ee'];
  const rankLabels  = ['1st', '2nd', '3rd', '4th', '5th'];
  const rankTitles  = ['Best Expansion Zone', 'Second-Best Zone', 'Third-Best Zone', 'Fourth-Best Zone', 'Fifth-Best Zone'];
  ...
  analysisPanel.innerHTML = `
    <div class="fade-in">
      ...
      <p class="panel-desc">${top_areas.length} area${top_areas.length !== 1 ? 's' : ''} ranked · ${candidates.length} analysis layer${candidates.length !== 1 ? 's' : ''} combined</p>
      ...
      <div class="tab-content active" id="tab-full">
        ...
        <p style="font-size:10px;color:var(--text-muted);margin-bottom:12px;line-height:1.5;">
          ${top_areas.length} expansion zone${top_areas.length !== 1 ? 's' : ''} found, ranked from best to least suitable across ${candidates.length} analysis layer${candidates.length !== 1 ? 's' : ''}. Each card shows how individual analyses score within that zone.
        </p>
...
function _renderExpansionMap(weighted_grid, top_areas) {
  ...
  if (top_areas && top_areas.length) {
    const colors  = ['#f1c40f', '#95a5a6', '#cd7f32', '#4a90e2', '#7b68ee'];
...
function renderAIRecommendations(tabEl, data) {
  ...
  tabEl.innerHTML = `
    ...
    ${legendHtml}
    ${sectionsHtml}
    <div style="margin-top:12px;border-top:1px solid var(--border);padding-top:8px;display:flex;align-items:center;justify-content:space-between;gap:8px;">
      <span style="font-size:10px;color:var(--text-muted);">Generated by Hadary AI · Based on ${lastAnalysisContext?.service_label || "analysis"} results${lastAnalysisContext?.grid ? " + grid cell data" : ""}</span>
      <button onclick="downloadAIReportPDF()" style="flex-shrink:0;display:flex;align-items:center;gap:5px;padding:5px 10px;font-size:11px;font-weight:600;color:var(--bg);background:var(--accent);border:none;border-radius:5px;cursor:pointer;letter-spacing:0.2px;" title="Download PDF report">
        <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M8 12l-4-4h2.5V2h3v6H12L8 12z"/><path d="M2 13h12v1.5H2z"/></svg>
        PDF Report
      </button>
    </div>`;
...
async function downloadAnalysisPDF() {
  ...
  // ── Capture map ───────────────────────────────────────────
  let mapDataUrl = null, mapPxW = 1, mapPxH = 1;
  try {
    const mapCanvas = document.querySelector("#map canvas");
    if (mapCanvas) {
      mapDataUrl = mapCanvas.toDataURL("image/jpeg", 0.85);
      mapPxW = mapCanvas.width  || 1;
      mapPxH = mapCanvas.height || 1;
    }
  } catch(e) {}
  ...
  // Map image — preserve aspect ratio, cap at 110 mm tall
  if (mapDataUrl) {
    const imgW = CW - 2;
    const imgH = Math.min(110, imgW * (mapPxH / mapPxW));
    ensurePage(imgH + 12);
    setFill(C.panel); rect(ML, y, CW, imgH + 4);
...
/* ============================================================
   AI REPORT PDF
   Downloads a styled PDF from the AI recommendations tab:
   cover → map snapshot → score gauge → data charts →
   AI sections (insights / warnings / recommendations) →
...
async function downloadAIReportPDF() {
  ...
  if (!tabEl || tabEl.querySelector(".ai-tab-placeholder")) {
    alert("Run an analysis and open the AI tab first.");
    return;
  }

  // Grab the last AI data blob stored on the element by renderAIRecommendations
  const ctx      = lastAnalysisContext;
  const service  = ctx?.service  || lastResultService || "analysis";
  const svcLabel = ctx?.service_label || service;

  // Read AI content directly from the rendered DOM so we match exactly what the user sees
  const headlineEl = tabEl.querySelector("div[style*='font-size:13px']");
  const headline   = headlineEl ? headlineEl.textContent.trim() : "AI Analysis";

  // Score
  const scoreValueEl = tabEl.querySelector(".insight-card .value");
  let   overallScore = null;
  if (scoreValueEl) {
    const m = scoreValueEl.textContent.match(/(\d+(?:\.\d+)?)\s*\/\s*100/);
    if (m) overallScore = parseFloat(m[1]);
  }

  // AI sections: each bordered-left div with a title + bullet list
  const aiSections = [];
  tabEl.querySelectorAll("div[style*='border-left']").forEach(el => {
    // skip the map-highlights box (it has a different background colour selector)
    if (el.style.background && el.style.background.includes("4cc2ff,0.06")) return;
    const titleEl = el.querySelector("div[style*='text-transform:uppercase']");
    const title   = titleEl ? titleEl.textContent.trim() : "";
    const items   = [];
    el.querySelectorAll("li").forEach(li => items.push(li.textContent.trim()));
    const borderColor = el.style.borderLeftColor || el.style.borderLeft || "";
    let type = "insight";
    if (borderColor.includes("231") || borderColor.includes("e74c3c") || title.includes("⚠")) type = "warning";
    else if (borderColor.includes("39") || borderColor.includes("27ae60") || title.includes("✅")) type = "recommendation";
    if (title || items.length) aiSections.push({ title, items, type });
  });

  // Map highlights (legend rows inside the "Map Highlights" box)
  ...
  // jsPDF built-in fonts only cover Latin-1 (cp1252).
  // Strip anything outside that range and replace known symbols with ASCII equivalents.
  function safe(str) {
    if (!str) return "";
    return str
      .replace(/💡/g, "[i]")
  ...
  function sectionHeader(text) {
    ensurePage(12);
    setFill(C.panel); rct(ML, y, CW, 9);
    setFill(C.accent); rct(ML, y, 3, 9);
    setTextC(C.white); doc.setFontSize(9); doc.setFont("helvetica","bold");
    doc.text(text, ML + 5, y + 6);
    y += 12;
  }
  ...
  function cardGrid(cards) {
    ...
    cards.forEach((card, i) => {
      ...
      const col = i % 2;
      const cx  = ML + col * (colW + 2);
      setFill(C.card); rct(cx, rowY, colW, cardH);
      setDraw(C.border); doc.setLineWidth(0.15); rct(cx, rowY, colW, cardH, "S");
      doc.setFont("helvetica","normal"); doc.setFontSize(7); setTextC(C.textMuted);
      doc.text(card.label, cx + 3, rowY + 3.5);
      doc.setFont("helvetica","bold"); doc.setFontSize(8); setTextC(C.textMain);
      const vl = doc.splitTextToSize(card.value, colW - 6);
  ...
  function scoreGauge(score) {
    ...
    ensurePage(24);
    const bx = ML, bw = CW, bh = 12, by = y + 4;
    setFill(C.panel); rct(ML, y, CW, 20);
    setTextC(C.textMuted); doc.setFont("helvetica","normal"); doc.setFontSize(7);
    doc.text("Overall AI Assessment Score", ML + 2, y + 3.5);
    setFill(C.card); rct(bx, by, bw, bh);
    setDraw(C.border); doc.setLineWidth(0.2); rct(bx, by, bw, bh, "S");
    const fillW = Math.max(2, (score / 100) * bw);
    let fillColor = score >= 75 ? C.green : score >= 50 ? [139,195,74] : score >= 25 ? C.orange : C.red;
    setFill(fillColor); rct(bx, by, fillW, bh);
    setDraw(C.border); doc.setLineWidth(0.3);
  ...
  // Horizontal progress bars for pct-based data
  function hBar(label, pct, color) {
    const w = Math.max(1, Math.min(100, pct || 0));
    const rowH = 8;
    ensurePage(rowH + 2);
    setFill(C.card); rct(ML, y, CW, rowH);
    // label
    doc.setFont("helvetica","normal"); doc.setFontSize(7); setTextC(C.textMuted);
    doc.text(safe(label), ML + 3, y + 5);
    // value
    doc.setFont("helvetica","bold"); doc.setFontSize(7); setTextC(C.textMain);
    doc.text(`${(+pct).toFixed(1)}%`, ML + CW - 3, y + 5, { align:"right" });
    // track + fill
    const trackX = ML + 3, trackY = y + 6, trackW = CW - 6, trackH = 1.5;
    setFill(C.border); rct(trackX, trackY, trackW, trackH);
    const rgb = typeof color === "string" ? hexToRgb(color) : color;
    setFill(rgb); rct(trackX, trackY, (w/100)*trackW, trackH);
    y += rowH + 1;
  }
  ...
  function barChart(data) {
    ...
    ensurePage(totalH + 4);
    setFill(C.panel); rct(ML, y, CW, totalH + 2);
    setTextC(C.textMuted); doc.setFont("helvetica","bold"); doc.setFontSize(7.5);
    doc.text(safe(chartTitle), ML + CW/2, y + 5, { align:"center" });
    const chartTop = y + 8, barAreaW = CW - 16, barAreaX = ML + 8;
    setDraw(C.border); doc.setLineWidth(0.15);
    [0, 0.5, 1].forEach(frac => {
      const lineY = chartTop + chartH * (1 - frac);
      doc.line(barAreaX, lineY, barAreaX + barAreaW, lineY);
      if (frac > 0) { setTextC(C.textMuted); doc.setFont("helvetica","normal"); doc.setFontSize(6);
        doc.text(`${Math.round(maxVal*frac)}`, barAreaX-1, lineY+1, { align:"right" }); }
    });
    const slotW = barAreaW / n;
    const barW  = Math.min(slotW * 0.62, 18);
    values.forEach((val, i) => {
      ...
      setFill([rgb[0]*0.4, rgb[1]*0.4, rgb[2]*0.4]); rct(bx+0.5, by+0.5, barW, barH);
      setFill(rgb); rct(bx, by, barW, barH);
      setTextC(C.textMain); doc.setFont("helvetica","bold"); doc.setFontSize(6.5);
      doc.text(`${val}`, bx+barW/2, by-1.2, { align:"center" });
      setTextC(C.textMuted); doc.setFont("helvetica","normal"); doc.setFontSize(6);
      const lbl = doc.splitTextToSize(safe(labels[i]), slotW-1);
      lbl.forEach((ln,li) => doc.text(safe(ln), bx+barW/2, chartTop+chartH+4+li*3.5, { align:"center" }));
    });
    setTextC(C.textMuted); doc.setFont("helvetica","italic"); doc.setFontSize(6);
    doc.text(`(${unit})`, barAreaX+barAreaW, chartTop+chartH+4, { align:"right" });
    y += totalH + 5;
  ...
  // Build service-specific chart descriptors from lastAnalysisContext (same data as renderAICharts)
  function aiChartData() {
    if (!ctx || !ctx.full_area) return [];
    const fa = ctx.full_area;
    switch (ctx.service) {
      case "public-transport":
        return [{ kind:"hbars", title:"Coverage Breakdown", bars:[
          { label:"Population Covered", pct: fa.coverage_pct||0,        color:"#4cc2ff" },
          { label:"Not Covered",        pct: 100-(fa.coverage_pct||0),  color:"#e74c3c" },
        ]}];
      case "vegetation":
        return [{ kind:"hbars", title:"Land Cover", bars:[
          { label:"Vegetated Area",  pct: fa.vegetation_pct||0,        color:"#27ae60" },
          { label:"Bare / Built-up", pct: 100-(fa.vegetation_pct||0),  color:"#e67e22" },
        ]}];
      case "ndvi":
        return [{ kind:"vchart", title:"NDVI Class Distribution", unit:"%",
          labels:["No Veg","Bare Soil","Moderate","Dense"],
          values:[fa.pct_no_veg||0, fa.pct_bare_soil||0, fa.pct_moderate||0, fa.pct_dense||0],
          colors:["#c0392b","#e67e22","#f1c40f","#27ae60"] }];
      case "heat-index":
        return [{ kind:"vchart", title:"Thermal Zone Distribution", unit:"%",
          labels:["Cool","Moderate","Hot","Extreme"],
          values:[fa.pct_cool||0, fa.pct_moderate||0, fa.pct_hot||0, fa.pct_extreme||0],
          colors:["#3498db","#f1c40f","#e67e22","#c0392b"] }];
      case "air-quality":
        return [{ kind:"vchart", title:"Air Quality Index Distribution", unit:"%",
          labels:["Good","Moderate","Sensitive","Unhealthy","V.Unhealthy","Hazardous"],
          values:[fa.good_pct||0, fa.moderate_pct||0, fa.sensitive_pct||0,
                  fa.unhealthy_pct||0, fa.very_unhealthy_pct||0, fa.hazardous_pct||0],
          colors:["#27ae60","#f1c40f","#e67e22","#e74c3c","#8e44ad","#6d2a2a"] }];
      case "traffic":
        return [{ kind:"hbars", title:"Road Network Composition", bars:[
          { label:"Primary Roads",            pct: fa.primary_pct||0,          color:"#4cc2ff" },
          { label:"Secondary Roads",          pct: fa.secondary_pct||0,        color:"#f1c40f" },
          { label:"Local Roads",              pct: fa.local_pct||0,            color:"#27ae60" },
          { label:"High Congestion Segments", pct: fa.high_congestion_pct||0,  color:"#e74c3c" },
        ]}];
      case "informal-settlement":
        return [{ kind:"hbars", title:"Irregularity Zones", bars:[
          { label:"High Irregularity",   pct: fa.high_pct||0,   color:"#e74c3c" },
          { label:"Medium Irregularity", pct: fa.medium_pct||0, color:"#e67e22" },
          { label:"Low Irregularity",    pct: fa.low_pct||0,    color:"#27ae60" },
        ]}];
      case "facility-accessibility": {
        const bars = [
          { label:"Accessible Population", pct: fa.coverage_pct||0,  color:"#27ae60" },
          { label:"Not Accessible",        pct: fa.uncovered_pct||(100-(fa.coverage_pct||0)), color:"#e74c3c" },
        ];
        const zColors = { "0-5 min":"#27ae60","5-10 min":"#2ecc71","10-15 min":"#f1c40f","15-20 min":"#e67e22",">20 min":"#e74c3c" };
        Object.entries(fa.zone_pcts||{}).forEach(([k,v]) =>
          bars.push({ label:k, pct:parseFloat(v)||0, color:zColors[k]||"#4cc2ff" }));
        return [{ kind:"hbars", title:"Accessibility Coverage", bars }];
      }
      case "urban-density": {
        const max = fa.max_density_pop_km2 || (fa.avg_density_pop_km2||0)*2 || 1;
        return [{ kind:"hbars", title:"Density Profile (relative)", bars:[
          { label:`Avg: ${(fa.avg_density_pop_km2||0).toFixed(0)} pop/km²`,    pct:(fa.avg_density_pop_km2||0)/max*100,    color:"#4cc2ff" },
          { label:`Median: ${(fa.median_density_pop_km2||0).toFixed(0)} pop/km²`, pct:(fa.median_density_pop_km2||0)/max*100, color:"#27ae60" },
          { label:"Target band midpoint (3,750)", pct:Math.min(100, 3750/max*100), color:"#f1c40f" },
        ]}];
      }
      case "crime": {
        const max = fa.max_density || (fa.avg_density||0)*2 || 1;
        return [{ kind:"hbars", title:"Crime Density Profile (relative)", bars:[
          { label:`Avg: ${(fa.avg_density||0).toFixed(2)} /km²`,  pct:(fa.avg_density||0)/max*100,  color:"#e67e22" },
          { label:`Peak: ${(fa.max_density||0).toFixed(2)} /km²`, pct:100,                           color:"#e74c3c" },
        ]}];
      }
      default: return [];
    }
  ...
  // ═══════════════════════════════════════════════════════
  setFill(C.bg);    rct(0, 0, PW, PH);
  setFill(C.panel); rct(0, 0, PW, 42);
  setFill(C.accent); rct(0, 39.5, PW, 2.5);
  ...
  // Report title
  setTextC(C.white); doc.setFont("helvetica","bold"); doc.setFontSize(15);
  doc.splitTextToSize("AI Analysis Report", CW - 20).forEach((ln, i) => doc.text(ln, ML, 28 + i*7));

  // Meta top-right
  setTextC(C.textMuted); doc.setFont("helvetica","normal"); doc.setFontSize(7.5);
  const dateStr = new Date().toLocaleDateString("en-US", { year:"numeric", month:"long", day:"numeric" });
  doc.text(`Generated: ${dateStr}`, PW - MR, 12, { align:"right" });
  doc.text(`Service: ${safe(svcLabel)}`,  PW - MR, 17, { align:"right" });
  if (ctx?.grid) { doc.text("Includes grid cell data", PW - MR, 22, { align:"right" }); }

  // AI badge strip
  setFill(C.accentDim); rct(ML, 46, CW, 7);
  setTextC(C.white); doc.setFont("helvetica","bold"); doc.setFontSize(7.5);
  doc.text(safe("* Powered by Hadary AI - Context-aware urban quality analysis"), ML + 3, 51);
  ...
  if (headline) {
    ensurePage(20);
    const hlLines = doc.splitTextToSize(safe(headline), CW);
    const hlH = hlLines.length * 5.5 + 6;
    setFill(C.panel); rct(ML, y, CW, hlH);
    setFill(C.accent); rct(ML, y, 3, hlH);
  ...
  if (overallScore !== null) {
    sectionHeader("OVERALL AI ASSESSMENT");
    scoreGauge(overallScore);
    y += 3;
  }
  ...
  if (charts.length) {
    sectionHeader("DATA VISUALIZATIONS");
    charts.forEach(ch => {
      if (ch.kind === "hbars") {
        ensurePage(14 + ch.bars.length * 10);
        // sub-title
        setTextC(C.textMuted); doc.setFont("helvetica","bold"); doc.setFontSize(7.5);
        doc.text(safe(ch.title), ML, y + 4); y += 7;
        ch.bars.forEach(b => hBar(b.label, b.pct, b.color));
        y += 3;
      } else if (ch.kind === "vchart") {
        barChart({ title: ch.title, labels: ch.labels, values: ch.values, colors: ch.colors, unit: ch.unit });
      }
    });
  }
  ...
  // SECTION — AI Insights
  // ═══════════════════════════════════════════════════════
  const typeOrder = ["warning","recommendation","insight","data","finding"];
  const sorted = [...aiSections].sort((a,b) => typeOrder.indexOf(a.type) - typeOrder.indexOf(b.type));

  if (sorted.length) {
    sectionHeader("AI INSIGHTS & RECOMMENDATIONS");
    sorted.forEach(sec => {
      if (!sec.title && !sec.items.length) return;
      const borderRgb = sec.type === "warning" ? C.red : sec.type === "recommendation" ? C.green : C.accent;
      const iconMap   = { insight:"[i]", warning:"[!]", recommendation:"[ok]", data:"[data]", finding:"[find]" };
      const icon      = iconMap[sec.type] || "[i]";

      // title bar — safe() cleans emoji that came from the DOM title text
      const itemsH = sec.items.length * 5.5 + 8;
      const titleH = 9;
      ensurePage(titleH + Math.min(itemsH, 30));

      setFill(C.panel); rct(ML, y, CW, titleH);
      setFill(borderRgb); rct(ML, y, 3, titleH);
      doc.setFont("helvetica","bold"); doc.setFontSize(7.5); setTextC(C.white);
      doc.text(`${icon}  ${safe(sec.title)}`, ML + 5, y + 6);
      y += titleH;

      // bullet items
      sec.items.forEach(item => {
        const lines = doc.splitTextToSize(`- ${safe(item)}`, CW - 10);
        const lineH = lines.length * 4.2 + 3;
        ensurePage(lineH);
        setFill(C.card); rct(ML, y, CW, lineH);
        setFill(borderRgb); rct(ML, y, 1.5, lineH);
        doc.setFont("helvetica","normal"); doc.setFontSize(7.5); setTextC(C.textMain);
        lines.forEach((ln, li) => doc.text(safe(ln), ML + 5, y + 3.5 + li * 4.2));
        y += lineH;
      });
      y += 4;
    });
  ...
  // ═══════════════════════════════════════════════════════
  // SECTION — Map Highlights Legend
  // ═══════════════════════════════════════════════════════
  if (highlights.length) {
    sectionHeader("MAP HIGHLIGHTS LEGEND");
    highlights.forEach(hl => {
      const descLines = doc.splitTextToSize(safe(hl.description || ""), CW - 24);
      const rowH = Math.max(10, descLines.length * 4 + 6);
      ensurePage(rowH + 2);
      setFill(C.card); rct(ML, y, CW, rowH);
      // colour swatch
      const swatchRgb = hexToRgb(hl.color || "#4cc2ff");
      setFill(swatchRgb); rct(ML + 3, y + (rowH-6)/2, 6, 6);
      // label
      doc.setFont("helvetica","bold"); doc.setFontSize(7.5); setTextC(C.textMain);
      doc.text(safe(hl.label || ""), ML + 12, y + 5);
      // description
      doc.setFont("helvetica","normal"); doc.setFontSize(7); setTextC(C.textMuted);
      descLines.forEach((ln, li) => doc.text(safe(ln), ML + 12, y + 9 + li * 4));
      y += rowH + 2;
    });
  }
  ...
  for (let p = 1; p <= pageCount; p++) {
    doc.setPage(p);
    setFill(C.panel); rct(0, PH-10, PW, 10);
    setFill(C.accentDim); rct(0, PH-10, PW, 0.5);
    setTextC(C.textMuted); doc.setFont("helvetica","normal"); doc.setFontSize(7);
    doc.text("Malaz - Urban Quality of Life Platform - AI Analysis Report", ML, PH-4);
    doc.text(`Page ${p} of ${pageCount}`, PW-MR, PH-4, { align:"right" });
  }

  const filename = `${service}_ai_report_${new Date().toISOString().slice(0,10)}.pdf`;
  doc.save(filename);
